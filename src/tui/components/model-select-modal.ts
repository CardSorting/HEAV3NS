...
export type CategoryTab = "all" | "openai-codex" | "custom";
...
export class ModelSelectModal implements Component, Focusable {
  ...
  constructor(
    ...
    const terraMatches = availableModels.filter(
      (m) => m.modelName === "gpt-5.6-terra" || m.modelName === "openai-codex/gpt-5.6-terra"
    );
  ...
  private createSelectListForCategory(category: CategoryTab): SelectList {
    ...
    if (category === "openai-codex") {
      filtered = this.availableModels.filter((m) => m.provider.toLowerCase() === "openai-codex" || m.modelName.startsWith("openai-codex/"));
  ...
  private renderModal(): void {
    ...
    const tabCodex = this.activeCategory === "openai-codex" ? "\x1b[1;36m[2: OPENAI CODEX]\x1b[0m" : "\x1b[90m[2: OPENAI CODEX]\x1b[0m";
    const tabCustom = this.activeCategory === "custom" ? "\x1b[1;36m[3: LOCAL / CUSTOM]\x1b[0m" : "\x1b[90m[3: LOCAL / CUSTOM]\x1b[0m";

    const tabsHeader = new Text(`${tabAll}  ${tabCodex}  ${tabCustom}`, 0, 0);
  ...
  handleInput(data: string): void {
    ...
    if (data === "\t") {
      const cats: CategoryTab[] = ["all", "openai-codex", "custom"];
