import type { ToolDefinition, ToolExecutionOptions } from "../../../core/contracts/tooling.contracts.js"
import type {
	ISwarmDelegator,
	SwarmGroupBy,
	SwarmSortBy,
 ...
 *
 * Model tool suite exposing bounded, isolated child execution plus swarm status,
 * metrics, and history operations. Child file changes remain staged for parent review.
 */
export class SwarmToolSuite {
	...
	constructor(delegator?: ISwarmDelegator, worktreeManager = new AnchoredWorktreeManager()) {
	...
	getTools(): ToolDefinition[] {
		return [
			...
			},
			{
				name: "delegate_abort",
				description:
					"Abort a pending or running task, cancel its provider request, discard its staged changes, and reclaim its reserved budget.",
				isMutating: true,
				parameters: {
			...
			},
			{
				...
				parameters: {
					status: {
						type: "string",
						description: "Optional status filter: 'pending', 'running', 'completed', 'failed', 'aborted'.",
					},
				},
				execute: async (args: Record<string, unknown>, cwd?: string) => {
			...
			},
			{
				...
				description:
					"Get aggregate telemetry: total subagents, tokens consumed, P50/P95 durations, and active worktrees.",
				parameters: {},
				execute: async (args: Record<string, unknown>, cwd?: string) => {
			...
			},
			{
				...
				description:
					"Request an isolated child task in a Git worktree. This CLI can stage child file edits but does not provision worktrees, so the request is rejected without starting a child.",
				parameters: {
			...
			},
			{
				...
				description:
					"Draft a simple phase outline for a goal. This is a planning preview only; it does not register tasks or start child agents.",
				parameters: {
			...
			},
			{
				...
				description: "Count active swarm records. This CLI runtime does not rebalance budgets or redistribute work.",
				parameters: {},
				execute: async (args: Record<string, unknown>, cwd?: string) => {
		...
		try {
			switch (name) {
				...
				case "delegate_batch": {
					...
					const result = await this.delegator.delegateBatch(manifests, parentSignal)
				...
				case "swarm_snapshot_restore": {
					...
					const target =
						typeof args.snapshotTick === "number"
							? recents.find((s) => s.snapshotTick === args.snapshotTick) || recents[recents.length - 1]
				...
				case "swarm_delegate_with_worktree": {
					return {
						success: false,
						error: "This CLI does not provision isolated Git worktrees. No child task was started.",
				...
				case "swarm_decompose_goal_to_swarm": {
					const goal = String(args.goal || "")
					if (!goal.trim()) return { success: false, error: "Provide a goal to preview a phase outline." }
					...
					return { success: true, execution: "plan_only", decomposedGoal: goal, subtasks }
				}

				case "swarm_rebalance_workers": {
					const tasks = this.delegator.listTasks("running")
					return { success: true, activeTaskCount: tasks.length, budgetRebalanced: false }
				}
