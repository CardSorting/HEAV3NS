...
3. **Unstructured Reasoning State**: Lack of structured in-memory reasoning traces, allocation-bounded circular streaming buffers, and checkpointed $O(1)$ state rollback.

## Decision
We implemented a allocation-bounded, typed, in-memory Streaming Reasoning Scrubber, Dynamic Timeout Floor & Adaptive Thinking Budget Substrate ($\mathcal{K}_{\text{think}}$ / Phase 102) for **LUMI-JOY**:

1. **`DeterministicReasoningScrubber`**:
   - In-memory allocation-bounded streaming state machine with chunk-boundary lookahead buffering.
...
## Consequences
- **Reasoning-output boundary**: Streaming chunk-boundary tag lookahead is tested to suppress the modeled internal-token patterns from visible prose; provider formats, malformed streams, and unmodeled cases require separate validation.
