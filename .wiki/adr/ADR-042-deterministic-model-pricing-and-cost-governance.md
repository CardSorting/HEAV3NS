...
2. **Network-Bound Pricing Lookups**: Live remote API calls blocked turns to look up token prices per call instead of using an in-memory allocation-bounded model pricing registry.
...
## Decision
We implemented a allocation-bounded, in-memory **Deterministic Model Pricing, Token Accounting & Cost Governance Substrate ($\mathcal{K}_{\text{cost}}$)** comprising five single-responsibility components:

1. **`DeterministicCostGovernor`** (`src/tooling/extensions/cost/deterministic-cost-governor.ts`):
   - In-memory allocation-bounded model pricing catalog with integer micro-cent ($10^{-6}\text{ USD}$) arithmetic to avoid floating-point rounding in represented values.
...
- **Precision**: Integer micro-cent arithmetic avoids floating-point accumulation in the represented arithmetic; it does not establish accounting correctness beyond that model.
