...
4. Active auth tokens and pending device code records lacked in-memory Broccolidb representations and checkpointed $O(1)$ state rollback.

## Decision
We implemented a typed, deterministic, allocation-bounded **OAuth2 PKCE Device Flow, Multi-Provider Identity Federation & Subscription Tier Governance Substrate ($\mathcal{K}_{\text{auth}}$)** for LUMI-JOY:
...
2. **Deterministic Auth Federator** (`src/tooling/extensions/auth/deterministic-auth-federator.ts`):
   - In-memory allocation-bounded engine generating RFC 7636 PKCE S256 verifier/challenge pairs, executing device flow handshakes, rotating token leases, and mapping subscription tiers to capability limits.
