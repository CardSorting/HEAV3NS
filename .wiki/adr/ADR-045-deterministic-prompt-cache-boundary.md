...
## Decision
We implemented a typed, deterministic, allocation-bounded **Prompt Cache Boundary, Progressive System Envelope & Reasoning Sanitizer Substrate ($\mathcal{K}_{\text{prompt}}$)** for LUMI-JOY:
...
2. **Deterministic Prompt Cacher** (`src/tooling/extensions/prompt/deterministic-prompt-cacher.ts`):
   - In-memory allocation-bounded prompt cache boundary calculator with 4-breakpoint layout, byte-stable static prefix isolation, `<think>` token scrubbing, and deterministic SHA-256 fingerprinting.
...
## Consequences
- Records byte-stable system-prompt prefixes with deterministic SHA-256 hashing in the covered prefix builder; provider behavior remains outside this record.
- Enforces strict 4-breakpoint cache layouts without in-place mutation hazards.
- Provides allocation-bounded `<think>` reasoning token extraction and sanitization.
