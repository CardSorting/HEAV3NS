...
## Decision
We implemented a typed, deterministic, allocation-bounded **Secret Redactor, Query Masker & Sensitive Path Safety Substrate ($\mathcal{K}_{\text{redact}}$)** for LUMI-JOY:
...
2. **Deterministic Secret Redactor** (`src/tooling/extensions/redaction/deterministic-secret-redactor.ts`):
   - In-memory allocation-bounded pre-compiled regex matching for vendor API keys (OpenAI, Anthropic, GitHub, AWS, Google, Stripe, Slack, HuggingFace), JWTs, PEM private keys, database URIs with password masking, sensitive URL query parameters, JSON body fields, and sensitive file path access gates.
...
## Consequences
- Redacts credentials in the covered tool outputs, logs, URL query parameters, and JSON payloads; this defense is not a complete secrecy guarantee.
