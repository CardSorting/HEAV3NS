...
| Required current capabilities | 566/566 | PASS |
| Composed runtime components | 566 | observed |
...
|---|---|---:|---|---|
| composition | Current evolution capabilities are composed | 0.05 ms | PASS | 566 required capabilities across 566 components |
| architecture | Core abstract contracts remain connected | 0.25 ms | PASS | agent, session, hands, ears, and tool-registry contracts verified |
| execution | Local frame commits an explicit successful outcome | 1.02 ms | PASS | frame 1 completed in 0.62 ms |
| state | Snapshot rewind restores frame and message state | 0.59 ms | PASS | restored frame 1 with 2 messages |
| governance | Completion gate fails closed without evaluated evidence | 0.52 ms | PASS | 5 fail-closed states rejected; 2/2 evaluated required criteria accepted |
...
| observability | Command output summaries remain bounded | 0.10 ms | PASS | head/tail output retention verified |
