...
## Decision
We implemented a typed, deterministic, allocation-bounded **Tool Execution Segmenter, Batch Parallelism Scheduler & Loop-Guardrail Substrate ($\mathcal{K}_{\text{loop}}$)** for LUMI-JOY:
...
2. **Deterministic Tool Segmenter** (`src/tooling/extensions/execution_guard/deterministic-tool-segmenter.ts`):
   - In-memory allocation-bounded batch segmentation planner and loop guardrail engine with idempotent/mutating taxonomy, SHA-256 call hashing, and anti-loop detection.
