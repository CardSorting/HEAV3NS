...
## Context
In ancestral architectures such as `hermes-agent` (`hermes_cli/backup.py`, `dump.py`, `session_export.py`, `session_export_html.py`, `session_export_md.py`, `sessions_cmd.py` — totaling 200+ KB, 5,000+ LOC), session exports and workspace backup packaging were tightly coupled to synchronous OS file system operations, heavy external subprocess execution (`zipfile`, `tarfile`), and unescaped HTML string concatenations vulnerable to cross-site scripting (XSS) and prompt injection hazards. Furthermore, these subsystems lacked deterministic in-memory models, SHA-256 content-addressable checksum validation, and checkpointed $O(1)$ state snapshotting.

## Decision
We implemented a allocation-bounded, typed, in-memory Multi-Format Session Export, Archive Packaging & Encrypted Backup Substrate ($\mathcal{K}_{\text{archive}}$ / Phase 99) for **LUMI-JOY**:

1. **`DeterministicSessionArchiver`**:
   - In-memory allocation-bounded session exporter producing sanitized GitHub-Flavored Markdown, standalone self-contained HTML5 documents with inline SVG and strict Content-Security-Policy (CSP) headers, deterministic JSONL message logs, and binary backup archive packages.
...
## Consequences
- **Security**: Defense-in-depth XSS and prompt-injection handling in HTML exports via strict entity escaping and a nonce-based CSP; this is not a security certification.
