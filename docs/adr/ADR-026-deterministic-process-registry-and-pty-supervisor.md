...
- **Date**: 2026-08-15
- **Technical Story**: Transmuting Hermes Agent's Interactive Process Registry & Background Task Supervisor (`tools/process_registry.py` [3,023 LOC] + `tools/terminal_tool.py` [3,852 LOC] — totaling 6,875 LOC, 307 KB) into a typed, deterministic, allocation-bounded **Interactive Process Registry, PTY Supervisor & Output Substrate ($\mathcal{K}_{\text{proc}}$ / Phase 74)** for LUMI-JOY via the AKD-DSO Osmosis Paradigm. Replaces 6,800+ lines of untyped Python OS subprocess code, mutable dictionary leaks, unmanaged JSON checkpoints, and garbage-collection-heavy string slicing with typed process lifecycles, allocation-bounded circular byte ring buffers, automated credential scrubbing, in-memory Broccolidb process substrates, and checkpointed $O(1)$ state rollback.
...
### 1. allocation-bounded Circular Byte Ring Buffer (`ProcessOutputRingBuffer`)
...
### 3. allocation-bounded In-Memory Process Substrate (`BroccoliProcessSubstrate`)
...
### 4. checkpointed Binary Snapshotting & $O(1)$ State Rollback (`ProcessSnapshotManager`)
...
- Manages subprocess spawning, PID tracking, interactive stdin streaming (`sendInput`), graceful termination (`killProcess`), watch pattern evaluation with cooldown limits, and auto-cleanup on engine shutdown with cleanup paths intended to limit zombie retention.
...
│   ├── process-output-ring-buffer.ts     # allocation-bounded circular 256KB byte ring buffer with ANSI stripping
...
│   └── process-snapshot-manager.ts       # checkpointed binary snapshotting & O(1) state rewind (<0.05 ms)
