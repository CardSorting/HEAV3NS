...
- **Date**: 2026-08-15
- **Technical Story**: Transmuting Hermes Agent's Agent Client Protocol adapter (`acp_adapter/` ~220 KB across 11 files: `server.py` 106 KB, `tools.py` 56 KB, `session.py` 27 KB, `edit_approval.py` 11 KB) into a typed, deterministic, allocation-bounded **Agent Client Protocol (ACP) IDE Bridge & Streaming JSON-RPC Server ($\mathcal{K}_{\text{acp}}$)** for LUMI-JOY via the AKD-DSO Osmosis Paradigm. Replaces 2,500 lines of untyped Python asyncio queues, threadpool executors, manual dictionary unpacking, and scattered approval hooks with typed JSON-RPC 2.0 codecs, interactive permission gates, in-memory Broccolidb session substrates, and checkpointed $O(1)$ state rollback.
...
2. **Untyped RPC Handlers & Dynamic Monkey-Patching**: Manual dictionary unpacking (`params.get(...)`) and dynamic runtime monkey-patching of tool registries per connection, weakening static type assumptions.
...
### 3. allocation-bounded In-Memory ACP Substrate (`BroccoliAcpSubstrate`)
...
### 4. checkpointed Binary Snapshotting & $O(1)$ State Rollback (`AcpSnapshotManager`)
...
│   └── acp-snapshot-manager.ts           # checkpointed binary snapshotting & O(1) state rewind
...
- **Determinism**: Session forking and frame-level state restoration are tested for the supported BroccoliDB paths; external providers, hosts, and unmodeled state can still vary.
