...
type SubscribeFn<TRequest, TResponse> = (
	request: TRequest,
...
type HealthListener = (
	snapshots: ReadonlyMap<string, GrpcSubscriptionSnapshot>,
) => void;
...
function logRecovery(
	...
	logDebug(
		`${QUIET_ERROR_PREFIX} Recovering "${key}" (${reason}, attempt ${attempt})`,
	);
}
...
export class GrpcSubscriptionRuntime {
	...
	acquire<TRequest, TResponse>(
		...
		let entry = this.streams.get(definition.key) as
		...
		entry.consumers.set(consumerId, {
			id: consumerId,
			handlers: consumer as GrpcSubscriptionConsumerOptions<TResponse>,
		});
	...
	getSnapshots(): ReadonlyMap<string, GrpcSubscriptionSnapshot> {
		return new Map(
			Array.from(this.streams.entries()).map(([key, entry]) => [
				key,
	...
	reconnectStale(reason: ReconnectReason = "visibility_restore"): void {
		for (const entry of this.streams.values()) {
			...
			if (
				!isReconnectEligible(entry.state) ||
				isTerminalFailedState(entry.state)
			) {
				continue;
			}
			this.reconnectTransport(entry, reason, { resetAttempt: true });
	...
	private connectTransport(entry: StreamEntry, reason: ReconnectReason): void {
		...
		logDebug(
			`${QUIET_ERROR_PREFIX} Connecting "${definition.key}" (${reason})`,
		);
	...
	private handleTransportEnd(
		...
		this.setState(
			entry,
			entry.reconnectAttempt > 0 ? "degraded" : "disconnected",
		);
		...
		for (const consumer of entry.consumers.values()) {
			if (error) {
				try {
					...
					console.error(
						`${QUIET_ERROR_PREFIX} onError callback failed:`,
						callbackError,
					);
				}
			}
	...
	private scheduleReconnect(
		...
		if (entry.reconnectAttempt >= policy.maxAttempts) {
			...
			logDebug(
				`${QUIET_ERROR_PREFIX} "${entry.definition.key}" entered failed state`,
			);
			return;
		}
	...
	private teardownTransport(
		...
		if (entry.cancel) {
			try {
				...
				console.error(
					`${QUIET_ERROR_PREFIX} Cancel failed for "${entry.definition.key}":`,
					error,
				);
			}
			entry.cancel = null;
	...
	private dispatchMessage(entry: StreamEntry, response: unknown): void {
		for (const consumer of entry.consumers.values()) {
			try {
				...
				console.error(
					`${QUIET_ERROR_PREFIX} onMessage callback failed for "${entry.definition.key}":`,
					error,
				);
			}
		}
	...
	private setState(entry: StreamEntry, state: SubscriptionHealthState): void {
		...
		for (const consumer of entry.consumers.values()) {
			try {
				...
				console.error(
					`${QUIET_ERROR_PREFIX} onHealthChange callback failed:`,
					error,
				);
			}
		}
