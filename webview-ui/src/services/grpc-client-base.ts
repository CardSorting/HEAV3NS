		...
		return new Promise((resolve, reject) => {
			...
			const fail = (error: unknown) => {
				...
				reject(
					toError(
						error,
						`gRPC request ${this.serviceName}.${methodName} failed`,
					),
				);
			};
			...
			const handleResponse = (event: MessageEvent) => {
				...
				if (
					...
					if (message.grpc_response.message) {
						...
						fail(
							new Error("Received ProtoBus response with no message or error."),
						);
					}
				}
			};
			...
			timeout = setTimeout(() => {
				fail(
					new Error(
						`Timed out waiting for ${this.serviceName}.${methodName} response.`,
					),
				);
			}, DEFAULT_UNARY_TIMEOUT_MS);

			try {
				const encodedMessage = PLATFORM_CONFIG.encodeMessage(
					request,
					encodeRequest,
				);
				...
				fail(
					toError(
						error,
						`Failed to post ${this.serviceName}.${methodName} request.`,
					),
				);
			}
		});
	...
	static makeStreamingRequest<TRequest, TResponse>(
		...
		const resetIdleTimeout = () => {
			...
			idleTimeout = setTimeout(() => {
				close({
					error: new Error(
						`Timed out waiting for ${this.serviceName}.${methodName} stream update.`,
					),
					sendCancel: true,
				});
			}, DEFAULT_STREAM_IDLE_TIMEOUT_MS);
			...
			if (
				...
				if (message.grpc_response.message) {
					...
					try {
						...
						close({
							error: toError(error, "Failed to decode streaming response."),
							sendCancel: true,
						});
						return;
					}
					...
					close({
						error: new Error(
							"Received ProtoBus stream response with no message or error.",
						),
						sendCancel: true,
					});
					return;
		...
		try {
			...
			close({
				error: toError(
					error,
					`Failed to start ${this.serviceName}.${methodName} stream.`,
				),
			});
		}
