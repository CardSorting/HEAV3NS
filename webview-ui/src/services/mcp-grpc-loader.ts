...
type McpSubscription<TResponse> = (
...
function loadMcpServiceClient(): Promise<typeof McpServiceClient> {
	if (!mcpClientPromise) {
		mcpClientPromise = import("./mcp-grpc-client").then(
...
function createLazySubscription<TResponse>(
	subscribe: (
		client: typeof McpServiceClient,
		request: EmptyRequest,
		callbacks: Callbacks<TResponse>,
	) => () => void,
...
): McpSubscription<McpServers>;
export function createLazyMcpSubscription(
	method: "subscribeToMcpMarketplaceCatalog",
): McpSubscription<McpMarketplaceCatalog>;
export function createLazyMcpSubscription(
	...
	return createLazySubscription<McpMarketplaceCatalog>(
