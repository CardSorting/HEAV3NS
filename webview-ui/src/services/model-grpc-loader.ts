...
type ModelSubscription = (
	...
	callbacks: Callbacks<OpenRouterCompatibleModelInfo>,
) => () => void;
...
export function loadModelsServiceClient(): Promise<typeof ModelsServiceClient> {
	if (!modelsClientPromise) {
		modelsClientPromise = import("./model-grpc-client").then(
			({ ModelsServiceClient }) => ModelsServiceClient,
		);
	}
	return modelsClientPromise;
}

export function createLazyModelSubscription(
	method: ModelServiceMethod,
): ModelSubscription {
...
export function convertModelResponse(response: OpenRouterCompatibleModelInfo) {
	return import(
		"../../../src/shared/proto-conversions/models/typeConversion"
	).then(({ fromProtobufModels }) => fromProtobufModels(response.models));
}
