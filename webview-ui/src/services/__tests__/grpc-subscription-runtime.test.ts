...
describe("GrpcSubscriptionRuntime closeout invariants", () => {
	...
	const createSubscribeMock = (
		behavior: "complete-on-first" | "error-always" | "message" | "slow-connect",
	) => {
		let connectCount = 0;
		const cancel = vi.fn();
		const subscribe = vi.fn(
			(
				...
				callbacks: Callbacks<{ ok: boolean }>,
			) => {
				connectCount += 1;
	...
	it("shares one transport per stable key (ref-counted)", async () => {
		...
		const releaseA = runtime.acquire(
			{ key: "shared", subscribe, request: {}, staleAfterMs: null },
			{ onMessage: vi.fn() },
		);
	...
	it("uses distinct transports for distinct keys", async () => {
		...
		runtime.acquire(
			{ key: "a", subscribe: subA.subscribe, request: {}, staleAfterMs: null },
			{ onMessage: vi.fn() },
		);
		runtime.acquire(
			{ key: "b", subscribe: subB.subscribe, request: {}, staleAfterMs: null },
			{ onMessage: vi.fn() },
		);
	...
	it("survives handler throws without tearing down transport", async () => {
		...
		runtime.acquire(
			{ key: "handler-throw", subscribe, request: {}, staleAfterMs: null },
			{ onMessage },
		);
	...
	it("visibility restore does not duplicate healthy connected streams", async () => {
		const { subscribe } = createSubscribeMock("message");
		runtime.acquire(
			{ key: "healthy", subscribe, request: {}, staleAfterMs: null },
			{ onMessage: vi.fn() },
		);
	...
	it("does not create duplicate streams after reconnect", async () => {
		const { subscribe } = createSubscribeMock("complete-on-first");
		const release = runtime.acquire(
			{ key: "reconnect", subscribe, request: {}, staleAfterMs: null },
			{ onMessage: vi.fn() },
		);

		await Promise.resolve();
		await vi.advanceTimersByTimeAsync(
			DEFAULT_RECONNECT_POLICY.initialDelayMs + 1000,
		);
