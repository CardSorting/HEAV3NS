...
import { buildRequestStartRuntimeState, RequestStartRuntimeProvider } from "@/components/chat/RequestStartRow"
...
export const MessagesArea = memo<MessagesAreaProps>(
	({ task, groupedMessages, modifiedMessages, rawMessages, scrollBehavior, chatState, messageHandlers }) => {
		const { taskLifecycleEvent } = useExtensionState()
		const lastRawMessage = useMemo(() => rawMessages.at(-1), [rawMessages])
		const isCompact = useIsCompact()
		const requestStartRuntimeState = useMemo(() => buildRequestStartRuntimeState(rawMessages), [rawMessages])
		...
		return (
			<section aria-label="Execution timeline" className="overflow-hidden flex flex-col h-full">
				...
				<div className="grow flex" ref={scrollContainerRef}>
					<RequestStartRuntimeProvider value={requestStartRuntimeState}>
						<Virtuoso
